...
impl BybitClient {
    ...
    pub async fn get_wallet_balance(&self, account_type: Option<&str>) -> Result<WalletBalanceResult> {
        ...
        debug!("Successfully fetched wallet balance for {} accounts (type: {})", result.list.len(), acc_type);
        Ok(result)
    }
    ...
    pub async fn get_instruments_info(&self, category: &str, limit: Option<u32>) -> Result<InstrumentsInfoResult> {
        ...
        info!("Successfully fetched {} instruments for category {}", result.list.len(), category);
        Ok(result)
    }
    ...
    pub async fn get_all_spot_instruments(&self) -> Result<Vec<InstrumentInfo>> {
        ...
        info!("Successfully fetched {} total spot instruments across {} pages", 
              all_instruments.len(), page);
    ...
    pub async fn place_order(&self, order_request: crate::models::PlaceOrderRequest) -> Result<crate::models::PlaceOrderResult> {
        ...
        let signature = self.generate_signature(timestamp, "POST", "/v5/order/create", "", &body)?;
        ...
        let api_response: crate::models::ApiResponse<serde_json::Value> = 
            ...
            .map_err(|e| anyhow::anyhow!("Failed to parse API response: {}. Response was: {}", e, response_text))?;

        if !api_response.is_success() {
            error!("Order placement failed. Request: {}", body);
            error!("API Error {}: {}", api_response.ret_code, api_response.ret_msg);
            return Err(anyhow::anyhow!("Order placement failed - API Error {}: {}", api_response.ret_code, api_response.ret_msg));
        }
        ...
        let typed_response: crate::models::ApiResponse<crate::models::PlaceOrderResult> = 
            serde_json::from_str(&response_text)
            .map_err(|e| anyhow::anyhow!("Failed to parse successful order response: {}. Response was: {}", e, response_text))?;
    ...
    pub async fn get_order(&self, category: &str, order_id: &str, symbol: &str) -> Result<crate::models::OrderInfo> {
        debug!("Getting order info: {}", order_id);
        
        let query_params = format!("category={}&orderId={}&symbol={}", category, order_id, symbol);
        ...
        debug!("Raw order status response: {}", serde_json::to_string_pretty(&response).unwrap_or_else(|_| "Failed to serialize".to_string()));
        ...
        match serde_json::from_value::<crate::models::OrderListResult>(response.clone()) {
            Ok(parsed) => {
                parsed.list.into_iter().next()
                    .ok_or_else(|| anyhow::anyhow!("Order not found in response"))
            },
            Err(parse_err) => {
                error!("Failed to parse order response: {}", parse_err);
                error!("Raw response was: {}", serde_json::to_string(&response).unwrap_or_else(|_| "Failed to serialize".to_string()));
